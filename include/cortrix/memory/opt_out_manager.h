...
//     check path are DEFERRED → integration.
...
/// Who triggered an opt-out (`opted_out_by` enum + request enum). Stamped on
...
///
/// `memory_opt_out.enabled` (cortrix.yaml) gates opt_out/opt_out_revoke: when disabled the
...
///
/// Standalone: every collaborator is an injected seam, so all three operations run
/// fully in unit tests against a mock store (+ an optional mock logger).
class OptOutManager {
public:
    /// @param store      opt-out seam (integration real MemoryStore adapter; UT mock)
    ...
    /// @param enabled    memory_opt_out.enabled (cortrix.yaml) — false ⇒ mutating calls 503.
    ...
    /// trigger — opt a session out of memory extraction. Validates session_id
    ...
    /// ARCH (the idempotent-200 variant is an HTTP-handler concern, integration).
    ...
    /// revoke — clear a session's opt-out (admin path). Only affects future
    /// interactions; already-skipped interactions are NOT re-processed (the
    ...
    /// `reason` is REQUIRED (requestBody) and recorded in the oplog summary.
    ...
    /// Pure. The boundary matrix only requires rejecting empty / structurally
    ...
    /// OptOutActor → the memory_sessions.opted_out_by string (enum). Pure.
...
/// the memory handler and is DEFERRED → integration. Declared here so the binding is a
