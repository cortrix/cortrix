...
// Store-backed adapters used by the MemoryExtractor
// + MemoryTransparency depend on through their seams (IMemoryBlockStore /
// IMemoryBlockLister). Memory facts are rows of the per-Unit `blocks` table
// (block_type = kBlockMemory) whose metadata_json carries the extraction fields
...
// so a freshly extracted memory is searchable). block_id (the memory ULID) maps to the
...
/// Contradiction-retrieval seam over the per-NS memory blocks. Standalone extraction
...
/// One user-fact row returned to the chat route (the stable interface
...
/// Query a user's active memory facts in one namespace (chat route). Returns
...
/// stable seam the chat router binds to; its shape is frozen here so the
