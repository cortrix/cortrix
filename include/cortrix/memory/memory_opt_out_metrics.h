...
/// OBS_SPEC / ARCH). The rename to `memory_immunity` is the Phase-2
...
/// `ns` label was removed from `extract_skipped_total` (v1.0.x V3 decision 10 —
/// `ns` is high-cardinality, forbidden). Per-NS data goes through the per-tenant
/// API, NOT this metric.
///
...
///
/// metric schema (3 rows, all counters):
...
public:
    /// triggered_by label for opt_out_total. Mirrors the `opted_out_by`
