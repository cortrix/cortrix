...
/// The 7 F13 error identities (F13 §9.2, topic 9 v1.0.6 final). Each maps to a
...
/// F02 reranker::RerankerErrorCode template exactly.
///
...
/// Total number of F13 error codes (F13 §9.2 = 7). Compile-time anchor for the
/// API-compatibility regression test (the set must not shrink).
constexpr int kF13ErrorCodeCount = 7;

/// Canonical, immutable attributes of one error code (F13 §9.2 columns).
struct F13ErrorInfo {
    const char* cx_code;                      ///< stable "CX_ERR_F13_*" string
    agent_friendly::ErrorCategory category;   ///< auth / permanent / transient
    bool retryable;
    std::optional<int> retry_after_ms;        ///< null for every F13 code (§9.2 retry_after_ms column = "-")
};
...
/// The structured_data keys a `code`'s error body MUST carry (F13 §9.2
...
/// Bridge an F13 error to a plain Status for the Result<T>/Status surface
/// (F-FREEZE-1). The StatusCode is the coarse mapping of `code`; the message is
/// prefixed with the CX_ERR_F13_* token ("CX_ERR_F13_X: detail") so the exact F13
