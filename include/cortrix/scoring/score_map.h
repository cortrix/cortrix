...
class ScoreMap {
public:
    /// Level → Score mapping (ruling: strict 5-level discrete). ARCH global SoT.
    static constexpr float kLevelScore[5] = {0.2f, 0.4f, 0.6f, 0.8f, 1.0f};

    /// ruling — Matrix multi-dimensional decision:
    ...
    ///   Level 0 (=0.2); aligned with ARCH metadata-locked semantic_score=0.2, consistent with.
    ...
    /// is the defensive bottom-out — ComputeLevel never produces an out-of-range
