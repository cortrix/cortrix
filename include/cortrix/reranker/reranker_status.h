...
///   2. structured status: the pgcortrix_status() reranker_status JSONB;
///   3. log: WARN on open, INFO on close (server-log spec).
///
/// Standalone: builds the JSONB + records to the in-process RerankerMetrics
/// + logs. Exposing it through the live pgcortrix_status() SQL function + the
/// metrics endpoint is cross-Feature wiring → integration.
...
public:
    ...
    /// Build the pgcortrix_status() reranker fields:
    ...
    // bundle (integration) can emit them verbatim.
