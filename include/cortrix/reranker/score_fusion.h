...
/// RerankerScoreFusion — F02-owned RRF score fusion (F02 §4.2-ter, V14 J2).
///
/// Fuses the cross-encoder precision score (`rerank_score`) with the F36 multi-
/// recall RRF score (`rrf_score`) into a single ordering score that
/// `OnnxReranker::Rerank` sorts on (§4.2-bis step 4+6). This is **F02's own**
/// fusion — distinct from F07 `SemanticScorer::ComputeFinalScore` (α-multiplier
...
/// (F43 Block hotness self-learning) + recency signals.
class RerankerScoreFusion {
public:
    /// V1 fusion weights (F02 §4.2-ter V1 contract). Kept as named constants so the
    ...
    static constexpr float kRrfWeight    = 0.3f;  ///< F36 multi-recall RRF weight

    /// Fuse the cross-encoder score with the RRF score into the final ordering
    /// score (normalized [0,1] when both inputs are in [0,1]).
    ///
    /// @param rerank_score  F02 cross-encoder score (ScoreBatch output, [0,1])
