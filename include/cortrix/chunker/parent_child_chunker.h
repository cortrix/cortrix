...
//   - parent/child IDs are time-ordered ULIDs, metadata is inherited per;
//   - when the estimated parent count exceeds max_parents_per_doc, the whole doc
//     degrades to a flat (single-layer) chunking (lock).
namespace cortrix::chunker {

class ParentChildChunker : public IChunker {
public:
    /// @param config  resolved ChunkerConfig (defaults = locks).
    ...
    ///   HfTokenizer-backed counter (integration). Must be deterministic + total.
...
private:
    // A unit of source text (one paragraph) carrying its provenance so a
    ...
    /// Flat fallback path: the whole document becomes one flat layer of
