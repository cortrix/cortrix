...
struct ChunkerConfig {
    ...
    int child_overlap = 20;                 ///< child overlap tokens, range [0, 100] (D1 lock)
    UnitLevel unit_level = UnitLevel::kParagraph;  ///< D2 lock
    int max_parents_per_doc = 10000;        ///< V1.0 OSS hard cap, range [100, 100000] (D3 lock)
    int fallback_to_flat_threshold = 10000; ///< flat-fallback trigger = D3 cap (D4 lock)
    int children_per_parent_for_rerank = 3; ///< C top-3 dedup, range [1, 10] (D5 lock)
};
