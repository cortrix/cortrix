...
#include "cortrix/spc/parser.h"  // cortrix::spc::DocumentMetadata (F06 §2.1 SoT)

// F34 Parent-Child Chunking — core data structures (detailed design § 2.2).
//
// Two-layer chunk model: a coarse Parent (1024t context block) + N fine Children
// (200t retrieval units). Only Children enter the vector index (F01 P-HNSW); the
...
/// at recall time. metadata carries doc-level fields + (post-F03) per-parent
/// NER/Summary; Children inherit it. (detailed design § 2.2)
struct ParentChunk {
    cortrix::id::ParentId parent_id;       ///< ULID, time-ordered
    cortrix::id::DocId    doc_id;          ///< upstream F06 document id
...
/// Child chunk — fine retrieval unit. `child_text` (≤ F02 max_seq_length) is the
...
/// here — it goes to F01 P-HNSW keyed by child_id with parent_id in vec metadata.
/// (detailed design § 2.2)
struct ChildChunk {
    ...
    cortrix::id::NamespaceId namespace_id;
    std::string child_text;                ///< ≤ F02 max_seq_length token text
...
/// carries the per-page warnings F06 reported (F34 inherits per-page tolerance,
/// § 4.5).
struct ChunkerStats {
    uint32_t total_pages = 0;              ///< pages received from F06
    uint32_t succeeded_pages = 0;          ///< pages with usable content
    uint32_t failed_pages = 0;             ///< pages F06 flagged failed (skipped)
