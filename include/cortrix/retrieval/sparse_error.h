...
/// The 5 F40 BGE-M3-sparse error identities (F40 §8, registered in ARCH §4.1.11).
...
/// (plus structured_data) into that boundary error. Mirrors the F02
...
enum class SparseErrorCode {
    ...
    kOnnxRuntimeInitFailed,  ///< F22-2 startup BGE-M3 model load failed
};

/// Total F40 error codes (F40 §8 = 5). Compile-time anchor for the
/// API-compatibility regression test (the set must not shrink).
constexpr int kSparseErrorCodeCount = 5;

/// Canonical, immutable attributes of one error code (F40 §8 columns).
...
/// The structured_data keys a `code`'s error body MUST carry (F40 §8
...
/// Bridge an F40 error to a plain Status for the Result<T>/Status surface
