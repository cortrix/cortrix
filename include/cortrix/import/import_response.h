...
/// the single source of truth even though the real HTTP routing is wired in integration.
...
/// success body for POST /api/v1/import/database. `estimated_rows` is the
/// SELECT COUNT(*) pre-estimate (R5); `progress_endpoint` / `cancel_endpoint` are
/// the self-links the Agent follows. error is always null here.
...
/// progress body for GET /api/v1/import/tasks/{task_id}/progress. On the FAILED
/// terminal state the error body is embedded; otherwise error is null.
