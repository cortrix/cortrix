...
constexpr int kImportErrorCodeCount = 7;
...
/// The "CX_ERR_IMPORT_*" string for `code` (convenience over GetImportErrorInfo).
...
agent_friendly::AgentFriendlyError MakeImportError(
    ImportErrorCode code,
    nlohmann::json structured_data = nlohmann::json::object(),
    const std::string& message = "");

/// ImportErrorCode → StatusCode coarse mapping (for the Result<T>/Status surface,
/// F-FREEZE-1). Rich category/retryable are preserved via the CX_ERR_* token +
/// MakeImportError() re-inflation at the API boundary.
...
public:
    explicit ImportException(ImportErrorCode code,
        ...
        : agent_friendly::AgentFriendlyException(
              MakeImportError(code, std::move(structured_data), message)),
          code_(code) {}

    ImportErrorCode code() const { return code_; }
