...
/// RAII owner of a per-Unit store.db connection (F05 §3.1 UnitResourceBundle.store_db).
///
/// The pool opens one of these per Unit during load and applies the F05 PRAGMAs
...
/// here: F05 owns the runtime *resource object* (the open connection), whereas
...
public:
    ...
    /// Apply the 6 F05 PRAGMAs (§7.3). No-op (returns Ok) when not open. PRAGMA
