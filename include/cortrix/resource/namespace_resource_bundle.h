...
/// Runtime resources for one Unit (F05 §3.1, topic 1). Owns the per-Unit P-HNSW
/// index handle, the per-NS F25 WriteCoordinator (which itself holds pending.wal),
/// and the per-Unit store.db connection. All three are released together when
/// the UnitResourceBundle is destroyed (UT2).
///
/// Naming (v1.0.2): the F25 resource is the real class `cortrix::store::Write-
...
struct UnitResourceBundle {
    ...
    std::unique_ptr<cortrix::store::IIndex> index;             ///< F01 P-HNSW handle
    ...
    std::unique_ptr<cortrix::store::WriteCoordinator> pwl;     ///< F25 coordinator (holds pending.wal)
    ...
    /// Resident memory estimate for this Unit (F05 §5 admission). Sums the index
    /// footprint (F01 IIndex::GetMemoryFootprintBytes, F05-2 hook) and the F25
    /// pending.wal size (WriteCoordinator::GetPwlSizeBytes, F05-3 hook). A null
    /// resource contributes 0 (write-only / partially-loaded bundles in tests).
    std::size_t MemoryEstimateBytes() const;
};

/// Runtime resources for one Namespace (F05 §3.1, topic 1). Phase 1: exactly one
/// Unit (`unit_bundles.size() == 1`, the 1:1 NS↔Unit mapping). Phase 2 (Overflow
/// mode, OPEN-3 / TD-F05-MULTI-UNIT) grows the collection to N Units — the vector
...
struct NamespaceResourceBundle {
    ...
    /// Total resident memory across all Units (F05 §3.1, used by the budget check).
