...
/// The 5 F35 Contextual Retrieval error identities (F35 §8, registered in ARCH
...
enum class ContextualErrorCode {
    ...
    kBudgetExceeded,   ///< per-NS soft quota exhausted (F35-8)
...
/// Total F35 error codes (F35 §8 = 5). Compile-time anchor for the
/// API-compatibility regression test (the set must not shrink).
constexpr int kContextualErrorCodeCount = 5;

/// Canonical, immutable attributes of one error code (F35 §8 columns).
...
/// The structured_data keys a `code`'s error body MUST carry (F35 §8
...
/// Bridge an F35 error to a plain Status for the Result<T>/Status surface
