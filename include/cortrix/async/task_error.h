...
/// identified by its CX_ERR_* code. So TaskErrorCode is the *enum of identities*,
/// and MakeTaskError() turns one (plus optional structured_data) into that
...
constexpr int kTaskErrorCodeCount = 11;
...
agent_friendly::AgentFriendlyError MakeTaskError(
    TaskErrorCode code,
    nlohmann::json structured_data = nlohmann::json::object(),
    const std::string& message = "");

/// TaskErrorCode → StatusCode coarse mapping (for the Result<T>/Status surface,
/// F-FREEZE-1). Rich category/retryable are preserved via the CX_ERR_* token +
/// MakeTaskError() re-inflation at the API boundary.
