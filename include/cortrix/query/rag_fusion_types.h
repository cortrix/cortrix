...
//. An unknown string deserializes to kParaphrase (defensive: never throws,
...
/// resolved global ← NS ← request via ConfigResolver<RagFusionConfig>.
///
...
struct RagFusionConfig {
    ...
    std::vector<VariantStrategy> variant_strategies = {     ///< topic: ARCH locked 3
    ...
    std::string model;                                      ///< per-call variant-LLM override; empty = client default
    ...
    /// v1.0.13: the anchored-fusion policy constants, previously
...
/// Validate a config's fields against the ranges. Returns false (and
...
/// query — the caller merges it (`all_queries = {query} + variants`).
