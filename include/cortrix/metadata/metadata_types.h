...
// rule-extracted structured metadata (no LLM dependency, lock). 26-field schema
...
// upload_timestamp / parse_time_ms (NOT the stale `lang`/`upload_time` of detailed design).
...
struct FileInfo {
    ...
    std::vector<std::string> tags;   ///< tags[] set by the business side at upload time (B' lock, set once)
...
struct GeneratorInput {
    ...
    /// B' lock: custom key-value set once by the business side at upload time (arbitrary JSON object). Empty object by default.
    nlohmann::json custom_metadata = nlohmann::json::object();
};

/// Generator output (detailed design). Carries the block plus the partial-success
/// warnings the body surfaces (coverage_ratio / fields_missing) so a partial
...
struct GeneratorOutput {
    ...
    /// coverage_ratio to the partial-success meta block.
    std::vector<std::string> missing_fields;
};

/// Total field count of the V1.0 metadata_json schema (detailed design — 23 + 1 +
