...
/// §7.2 "note": CX_ERR_OPLOG_QUOTA_EXCEEDED is deliberately NOT a member — the
...
/// CX_ERR_OPLOG_CLEANUP_RUNNING base backoff (§7.2: "5000 + jitter"). The base is
...
struct OplogErrorInfo {
    ...
    bool retryable;
    std::optional<int> retry_after_ms;        ///< null unless retryable per §7.2
};
...
/// (the §7.2 required keys are the caller's responsibility at each call site) and
