...
def test_error_table_has_seven_runtime_codes():
    ...
    assert "CX_ERR_AGENT_RAG_FAILED" in ERROR_TABLE
    assert "CX_ERR_AGENT_SESSION_NOT_FOUND" in ERROR_TABLE
    assert "CX_ERR_AGENT_INTERACTION_LOG_FAILED" in ERROR_TABLE
...
def test_startup_table_has_five_codes_independent_group():
    ...
    assert "CX_ERR_AGENT_CORTRIX_SERVER_UNREACHABLE" in STARTUP_ERROR_TABLE
...
def test_to_dict_emits_exactly_the_four_plus_two_fields():
    ...
    err = AgentError("CX_ERR_AGENT_RAG_FAILED", structured_data={"cortrix_server_error": "boom"})
    payload = err.to_dict()
    ...
    assert body["code"] == "CX_ERR_AGENT_RAG_FAILED"
...
def test_known_code_defaults_category_retryable_and_status_from_registry():
    err = AgentError("CX_ERR_AGENT_LLM_TIMEOUT")
...
def test_session_not_found_is_permanent_404():
    err = AgentError("CX_ERR_AGENT_SESSION_NOT_FOUND", structured_data={"session_id": "x"})
...
def test_unknown_code_falls_back_to_permanent_500():
    err = AgentError("CX_ERR_AGENT_TOTALLY_UNKNOWN")
    assert err.category == "permanent"
    assert err.retryable is False
    assert err.http_status == 500
    # message defaults to the code itself when not in the registry.
    assert err.message == "CX_ERR_AGENT_TOTALLY_UNKNOWN"


def test_agent_error_response_helper_matches_to_dict():
    direct = AgentError("CX_ERR_AGENT_LLM_QUOTA_EXCEEDED").to_dict()
    helper = agent_error_response("CX_ERR_AGENT_LLM_QUOTA_EXCEEDED")
    assert direct == helper


def test_agent_error_is_raisable():
    with pytest.raises(AgentError) as exc_info:
        raise AgentError("CX_ERR_AGENT_LLM_UNAVAILABLE")
    assert exc_info.value.code == "CX_ERR_AGENT_LLM_UNAVAILABLE"
