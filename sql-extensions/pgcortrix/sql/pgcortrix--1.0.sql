...
--
-- SoT: design/features/the pgcortrix design (signatures, GUC,
...
-- code — that is the V3+ roadmap. The Python logic is kept in the helper
-- module (not inlined here) so it is unit-testable off a live PG (mock urllib),
-- which is how the standalone DoD is met (L1-α briefing).
--
...
-- pgcortrix_batch_submit (batch submit, impl layer) wraps
...
-- ===========================================================================
--  Composite return types (4)
-- ===========================================================================
-- Four SETOF-returning functions need a composite row type each. Column sets
-- mirror the HTTP response schemas these functions wrap (pgcortrix → ARCH).
...
CREATE TYPE pgcortrix_memory_result AS (
    ...
    final_score FLOAT        -- memory decay: raw * decay_factor (ranking key, design)
);

-- v1.0.1: interaction_log list type. memory isolation forces per-user isolation,
-- so pgcortrix_list_interactions always carries user_id (fn 5).
...
-- IP blocklist on every call, so this is defense-in-depth.
--
...
-- 3.bis Batch document submit (impl layer).
...
-- (batch submit — V1 is always async).
...
-- 5. interaction_log list (v1.0.2 — V3 decision 4 "combined" shape).
-- Memory isolation three-way parity = user_id mandatory (security) + filter capability aligned
-- with agent trace HTTP API (function). Three-way parity with MCP
-- cortrix_list_interactions + HTTP GET /memory/interactions.
--   filter JSONB whitelist: session_id / namespace_id / from_ts / to_ts
--   / sort_order. Anything else → CX_ERR_PGCORTRIX_INVALID_FILTER.
CREATE FUNCTION pgcortrix_list_interactions(
    namespace TEXT,
    user_id   TEXT,                  -- memory isolation: mandatory positional (no NULL/default)
...
-- ===========================================================================
--  Helper functions (2)  — main = 1..6, helpers numbered from 7
-- ===========================================================================
...
-- 7. Status + connection diagnostics (V23: unified status() JSONB, pg_stat_*
