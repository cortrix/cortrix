...
Status DocSummarySchemaProvider::Migrate(sqlite3* db, int from_ver, int to_ver) {
    ...
    if (from_ver == 0 && to_ver == 1) {
        if (db == nullptr) {
            return DocSummaryStatus(DocSummaryErrorCode::kSchemaVersionMismatch,
                                    "doc_summary Migrate got null db");
        }
        char* err = nullptr;
    ...
    return DocSummaryStatus(
        ...
        "doc_summary unsupported migration " + std::to_string(from_ver) + " -> " +
