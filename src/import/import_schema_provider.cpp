...
Status ImportSchemaProvider::Migrate(sqlite3* db, int from_ver, int to_ver) {
    ...
    if (from_ver == 0 && to_ver == kImportSchemaVersion) {
        char* err = nullptr;
        if (sqlite3_exec(db, kImportSchemaSql, nullptr, nullptr, &err) != SQLITE_OK) {
    ...
    return Status::InvalidArgument(
        "CX_ERR_SCHEMA_VERSION_MISMATCH: db_import unsupported migration " +
