...
L1SerializeResult L1SerializeSparseVec(const SparseVector& vec, int retries) {
    L1SerializeResult r;
    // Dead chunk: no sparse signal → write NULL, clear the flag, no error.
    if (vec.empty()) {
        r.serialized = false;
    ...
    //: try to serialize; on failure retry up to `retries` more times, then
...
SparseExplain DecideL2Fallback(bool sparse_available, int resolved_top_k) {
    ...
    if (sparse_available) {
        ...
        // L2: transparent degrade to dense + FTS5 (+ hype) — drop sparse.
...
FivePathInput DropSparsePath(FivePathInput input) {
    input.sparse.clear();  //: 5-path → 4-path (no sparse)
    return input;
}
