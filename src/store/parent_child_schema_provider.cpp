...
Status Exec(sqlite3* db, const char* ddl, const char* what) {
    ...
    if (sqlite3_exec(db, ddl, nullptr, nullptr, &err) != SQLITE_OK) {
        ...
        return Status::Internal(
            std::string("CX_ERR_SCHEMA_MIGRATION_FAILED: parent-child ") + what + ": " + detail);
    }
    return Status::Ok();
}
...
Status ParentChildSchemaProvider::Migrate(sqlite3* db, int from_ver, int to_ver) {
    const bool init = (from_ver == 0 && to_ver == 1);
    if (!init && from_ver != to_ver) {
        return Status::InvalidArgument(
            "CX_ERR_SCHEMA_VERSION_MISMATCH: parent_child unsupported migration " +
