...
std::string AgentTraceCleanupRegistrar::FormatIso8601Utc(int64_t unix_ms) {
...
AgentTraceCleanupRegistrar::AgentTraceCleanupRegistrar(std::shared_ptr<IAgentTraceWriter> writer,
                                         sqlite3* db,
                                         std::shared_ptr<IGlobalConfig> config)
    : writer_(std::move(writer)), db_(db), config_(std::move(config)) {}

int AgentTraceCleanupRegistrar::CleanupInteractionLog() {
...
void AgentTraceCleanupRegistrar::RegisterAgentTrace(observability::CleanupScheduler& scheduler) {
