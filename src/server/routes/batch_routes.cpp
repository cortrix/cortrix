...
bool ParseBatchRequest(const httplib::Request& req, httplib::Response& res,
    ...
    for (const auto& item : body["documents"]) {
        ...
        // selection (batch submit documents[] item; ".txt" fallback applied
    ...
    // options (batch submit): async defaults true (must be true in V1);
    ...
    // topic 3 — V1 is always async; an explicit async=false is unsupported.
    if (!out->async) {
        WriteJsonError(res,
...
void RegisterBatchRoutes(httplib::Server& server,
                         ...
                         ApiKeyAuth& auth) {
    // POST /api/v1/documents/batch — Agent-first batch submit.
