...
constexpr double kDurBounds[F42Metrics::kNumDurBuckets] =
    {1.0, 5.0, 15.0, 30.0, 60.0, 300.0, 900.0, 1800.0};
constexpr const char* kDurBoundStr[F42Metrics::kNumDurBuckets] =
...
int DurBucketIndex(double seconds) {
    for (int j = 0; j < F42Metrics::kNumDurBuckets; ++j) {
        if (seconds <= kDurBounds[j]) return j;
    }
    return F42Metrics::kNumDurBuckets;  // +Inf
}
...
void F42Metrics::RecordSubmitted(TaskType task_type) {
    submitted_[TaskTypeIndex(task_type)].fetch_add(1, std::memory_order_relaxed);
}

uint64_t F42Metrics::SubmittedCount(TaskType task_type) const {
    return submitted_[TaskTypeIndex(task_type)].load(std::memory_order_relaxed);
}

void F42Metrics::RecordCompleted(TaskType task_type, CompletionStatus status) {
    completed_[TaskTypeIndex(task_type)][static_cast<int>(status)].fetch_add(
        1, std::memory_order_relaxed);
}

uint64_t F42Metrics::CompletedCount(TaskType task_type, CompletionStatus status) const {
    return completed_[TaskTypeIndex(task_type)][static_cast<int>(status)].load(
        std::memory_order_relaxed);
}

void F42Metrics::ObserveDuration(TaskType task_type, double seconds) {
...
const char* ToString(F42Metrics::CompletionStatus status) {
    switch (status) {
        case F42Metrics::CompletionStatus::kSuccess:   return "success";
        case F42Metrics::CompletionStatus::kFailed:    return "failed";
        case F42Metrics::CompletionStatus::kCancelled: return "cancelled";
        case F42Metrics::CompletionStatus::kTimeout:   return "timeout";
    }
    return "unknown";
}
...
const char* ToString(F42Metrics::QueueState state) {
    switch (state) {
        case F42Metrics::QueueState::kQueued:     return "queued";
        case F42Metrics::QueueState::kProcessing: return "processing";
    }
    return "unknown";
}
...
const char* ToString(F42Metrics::CancelPhase phase) {
    switch (phase) {
        case F42Metrics::CancelPhase::kPreDequeue:    return "pre_dequeue";
        case F42Metrics::CancelPhase::kMidProcessing: return "mid_processing";
        case F42Metrics::CancelPhase::kPostChunkIdx:  return "post_chunk_idx";
    }
    return "unknown";
}
