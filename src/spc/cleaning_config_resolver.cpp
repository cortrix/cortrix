...
CleaningConfig LoadGlobalCleaningConfig(const IGlobalConfig* global) {
    CleaningConfig cfg;  // struct defaults (cleaning_types.h §3.1)
...
Result<CleaningConfig> ResolveCleaningConfig(const CleaningConfig& global,
    ...
    if (!merged.ok()) {
        // Re-badge the catalog merge failure as the cleaning-native §5.1 error so the
    ...
    // §3.4 line 176: max_chunk_chars / plugin_timeout_ms are system-resource level
