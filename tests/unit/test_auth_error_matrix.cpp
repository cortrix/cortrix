#include <gtest/gtest.h>

#include <regex>
#include <set>
#include <string>
#include <vector>

#include "cortrix/agent_friendly/error.h"
#include "cortrix/auth/auth_error.h"

// Exhaustive parameterized error-registry sweep for auth (codes).
// Distinct suite name (AuthErrorMatrix) from test_auth_error.cpp.
namespace cortrix::auth {
namespace {

using agent_friendly::ErrorCategory;

const AuthErrorCode kAll[] = {
    AuthErrorCode::kEmailAlreadyExists,
    AuthErrorCode::kInvalidCredentials,
    AuthErrorCode::kAccountLocked,
    AuthErrorCode::kAccountDisabled,
    AuthErrorCode::kTokenExpired,
    AuthErrorCode::kTokenRevoked,
    AuthErrorCode::kInvalidRefreshToken,
    AuthErrorCode::kTokenVerificationFailed,
    AuthErrorCode::kInvalidResetCode,
    AuthErrorCode::kBootstrapTokenInvalid,
    AuthErrorCode::kInvalidApiKey,
    AuthErrorCode::kAdminRequired,
    AuthErrorCode::kEmailSendFailed,
    AuthErrorCode::kBcryptTimeout,
    AuthErrorCode::kJwtInitFailed,
    AuthErrorCode::kUserNotFound,
    AuthErrorCode::kUserEmailExists,
    AuthErrorCode::kUserValidation,
    AuthErrorCode::kInvalidRequest,
    AuthErrorCode::kUnauthorized,
    AuthErrorCode::kNotFound,
    AuthErrorCode::kRateLimited,
    AuthErrorCode::kInternalError,
    AuthErrorCode::kServiceUnavailable,
};

bool IsValidCategory(ErrorCategory c) {
    return c == ErrorCategory::kAuth || c == ErrorCategory::kQuota ||
           c == ErrorCategory::kTransient || c == ErrorCategory::kPermanent ||
           c == ErrorCategory::kTimeout;
}

class AuthErrorMatrix : public ::testing::TestWithParam<AuthErrorCode> {};

TEST_P(AuthErrorMatrix, CodeStringFormat) {
    const AuthErrorCode code = GetParam();
    const std::string s = AuthErrorCodeString(code);
    EXPECT_FALSE(s.empty());
    EXPECT_EQ(s.rfind("CX_ERR_", 0), 0u) << s;
    EXPECT_TRUE(std::regex_match(s, std::regex("^CX_ERR_[A-Z0-9_]+$"))) << s;
    EXPECT_STREQ(GetAuthErrorInfo(code).cx_code, s.c_str());
}

TEST_P(AuthErrorMatrix, CategoryAndRetryInvariant) {
    const AuthErrorCode code = GetParam();
    const AuthErrorInfo& info = GetAuthErrorInfo(code);
    EXPECT_TRUE(IsValidCategory(info.category));
    // Non-retryable codes carry no static retry hint. Some retryable codes
    // (kAccountLocked / kRateLimited) carry a null static value because the live
    // remaining-lockout ms is supplied per-call via retry_after_override, so the
    // converse direction is not asserted here.
    if (!info.retryable) {
        EXPECT_FALSE(info.retry_after_ms.has_value()) << info.cx_code;
    }
    if (info.retry_after_ms.has_value()) {
        EXPECT_GT(*info.retry_after_ms, 0) << info.cx_code;
    }
}

TEST_P(AuthErrorMatrix, RequiredStructuredDataArms) {
    const AuthErrorCode code = GetParam();
    const std::vector<std::string>& keys = RequiredStructuredDataKeys(code);

    nlohmann::json full = nlohmann::json::object();
    for (const std::string& k : keys) full[k] = "v";
    EXPECT_TRUE(HasRequiredStructuredData(code, full));

    if (!keys.empty()) {
        nlohmann::json missing = full;
        missing.erase(keys.front());
        EXPECT_FALSE(HasRequiredStructuredData(code, missing));
    }

    EXPECT_EQ(HasRequiredStructuredData(code, nlohmann::json("not-an-object")),
              keys.empty());
}

TEST_P(AuthErrorMatrix, MakeRoundTripAndJson) {
    const AuthErrorCode code = GetParam();
    const AuthErrorInfo& info = GetAuthErrorInfo(code);

    nlohmann::json sd = nlohmann::json::object();
    for (const std::string& k : RequiredStructuredDataKeys(code)) sd[k] = "v";

    auto err = MakeAuthError(code, sd, "human detail");
    EXPECT_EQ(err.code, info.cx_code);
    EXPECT_EQ(err.message, "human detail");
    EXPECT_EQ(err.retryable, info.retryable);
    EXPECT_EQ(err.category, info.category);
    EXPECT_EQ(err.retry_after_ms, info.retry_after_ms);

    nlohmann::json j = agent_friendly::ToJson(err);
    EXPECT_EQ(j["code"], std::string(info.cx_code));
    EXPECT_EQ(j["retryable"], info.retryable);
    EXPECT_EQ(j["category"], std::string(agent_friendly::ToString(info.category)));
    EXPECT_EQ(j["retry_after_ms"].is_null(), !info.retry_after_ms.has_value());
}

TEST_P(AuthErrorMatrix, RetryAfterOverrideWins) {
    const AuthErrorCode code = GetParam();
    auto err = MakeAuthError(code, nlohmann::json::object(), "", /*override=*/12345);
    ASSERT_TRUE(err.retry_after_ms.has_value());
    EXPECT_EQ(*err.retry_after_ms, 12345);
}

TEST_P(AuthErrorMatrix, EmptyMessageFallsBackToCode) {
    const AuthErrorCode code = GetParam();
    auto err = MakeAuthError(code);
    EXPECT_EQ(err.message, std::string(AuthErrorCodeString(code)));
}

TEST_P(AuthErrorMatrix, StatusBridge) {
    const AuthErrorCode code = GetParam();
    const StatusCode mapped = AuthErrorToStatusCode(code);
    EXPECT_NE(mapped, StatusCode::kOk);

    Status s = AuthStatus(code, "detail-x");
    EXPECT_FALSE(s.ok());
    EXPECT_EQ(s.code(), mapped);
    EXPECT_NE(s.message().find(AuthErrorCodeString(code)), std::string::npos);
    EXPECT_NE(s.message().find("detail-x"), std::string::npos);
}

TEST(AuthErrorMatrixAggregate, CountAndUniqueCodes) {
    EXPECT_EQ(kAuthErrorCodeCount, 24);
    EXPECT_EQ(std::size(kAll), static_cast<size_t>(kAuthErrorCodeCount));
    std::set<std::string> seen;
    for (AuthErrorCode c : kAll) {
        EXPECT_TRUE(seen.insert(AuthErrorCodeString(c)).second);
    }
    EXPECT_EQ(seen.size(), static_cast<size_t>(kAuthErrorCodeCount));
}

INSTANTIATE_TEST_SUITE_P(AllCodes, AuthErrorMatrix, ::testing::ValuesIn(kAll));

}  // namespace
}  // namespace cortrix::auth
