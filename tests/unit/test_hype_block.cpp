...
TEST(HypeBlockTest, MetadataJsonCustomProvider) {
...
TEST(HypeBlockTest, BuildsBlockWithType16) {
...
TEST(HypeBlockTest, BlockContentIsQuestionText) {
...
TEST(HypeBlockTest, BlockEnrichmentSourceIsLlm) {
    HypeQuestion q = MakeQ("q", 0, "c", "p");
    auto blob = BuildHypeQuestionBlock(q, "v1", "gpt-4o-mini", 0.0, "t");
    const cortrix_block_header_t* hdr = nullptr;
...
TEST(HypeBlockTest, BlockCrcDetectsTamper) {
    HypeQuestion q = MakeQ("q", 0, "c", "p");
    auto blob = BuildHypeQuestionBlock(q, "v1", "gpt-4o-mini", 0.0, "t");
...
TEST(HypeBlockTest, FilledEmbeddingFlowsIntoBlockVectorDim) {
