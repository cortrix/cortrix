...
// F38 S5 — HyPE error model (§7, ARCH §4.1.11) — template A registry. Pins the 6
...
TEST(F38HypeErrorTest, CountIsSix) {
...
TEST(F38HypeErrorTest, CategoryAndRetryMatchDesign) {
...
TEST(F38HypeErrorTest, RetryableImpliesRetryAfterSet) {
    for (auto code : kAll) {
...
TEST(F38HypeErrorTest, RequiredStructuredDataKeys) {
...
TEST(F38HypeErrorTest, HasRequiredStructuredData) {
...
TEST(F38HypeErrorTest, MakeHypeErrorFillsFromRegistry) {
...
TEST(F38HypeErrorTest, MakeHypeErrorSerializesToAgentFriendlyBody) {
...
TEST(F38HypeErrorTest, StatusCodeMapping) {
