...
TEST(ContextualEnricherTest, UsableViaBasePointer) {
...
TEST(ContextualEnricherTest, UnavailableWithoutClient) {
    ContextualRetrievalEnricher e(ContextualRetrievalConfig{}, nullptr, MakeEmbedder());
    EXPECT_FALSE(e.IsAvailable());
}

TEST(ContextualEnricherTest, UnavailableWhenDisabled) {
...
TEST(ContextualEnricherTest, GeneratesPrefixText) {
...
TEST(ContextualEnricherTest, GenerateNullClientIsStartupNoLlm) {
    ContextualRetrievalEnricher e(ContextualRetrievalConfig{}, nullptr, MakeEmbedder());
...
TEST(ContextualEnricherTest, GenerateLlmFailureIsLlmFailed) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(ContextualEnricherTest, GenerateOverlongOutputIsPromptInjection) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(ContextualEnricherTest, GenerateNaturalLengthOutputAcceptedAtDefaultGuard) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    EXPECT_CALL(*llm, Chat(_, _))
...
TEST(ContextualEnricherTest, GeneratePassesCallConfig) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(ContextualEnricherTest, DefaultPromptHasAnthropicStructure) {
    ContextualRetrievalEnricher e(ContextualRetrievalConfig{},
                                  std::make_shared<llm::MockLlmClient>(), MakeEmbedder());
...
TEST(ContextualEnricherTest, DefaultPromptEmptyBoundariesRenderEmpty) {
    ContextualRetrievalEnricher e(ContextualRetrievalConfig{},
                                  std::make_shared<llm::MockLlmClient>(), MakeEmbedder());
...
TEST(ContextualEnricherTest, NsOverrideTemplateSubstitutesPlaceholders) {
    ContextualRetrievalConfig cfg;
    cfg.prompt_template =
...
TEST(ContextualEnricherTest, EnrichSuccessPopulatesAllThreeFields) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(ContextualEnricherTest, EnrichNullClientSkipsNoLlm) {
    ContextualRetrievalEnricher e(ContextualRetrievalConfig{}, nullptr, MakeEmbedder());
...
TEST(ContextualEnricherTest, EnrichDisabledSkipsNoLlm) {
    ContextualRetrievalConfig cfg;
    cfg.enabled = false;
    ContextualRetrievalEnricher e(cfg, std::make_shared<llm::MockLlmClient>(),
                                  MakeEmbedder());
...
TEST(ContextualEnricherTest, EnrichLlmFailureDegradesToFailed) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    llm::ChatCompletionResponse fail;
...
TEST(ContextualEnricherTest, EnrichNullEmbedderDegradesButKeepsText) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(ContextualEnricherTest, EnrichEmbedderFailureDegradesToFailed) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    EXPECT_CALL(*llm, Chat(_, _)).WillOnce(Return(OkChat("PFX")));
    auto emb = MakeEmbedder();
    emb->fail = true;
...
TEST(ContextualEnricherTest, EnrichRecordsChunkMetric) {
...
TEST(ContextualEnricherTest, EnrichBatchRunsPerContext) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(ContextualEnricherTest, EnrichBatchNullDocMetadataUsesEmpty) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    EXPECT_CALL(*llm, Chat(_, _)).WillRepeatedly(Return(OkChat("PFX")));
    ContextualRetrievalEnricher e(ContextualRetrievalConfig{}, llm, MakeEmbedder());
...
TEST(ContextualConfigTest, ResolveReadsGlobalLayer) {
...
TEST(ContextualConfigTest, ResolveClampsMaxOutputTokens) {
    InMemoryGlobalConfig gc;
...
TEST(ContextualConfigTest, ResolveUnparseableValueKeepsDefault) {
    InMemoryGlobalConfig gc;
...
TEST(ContextualConfigTest, ResolveReadsTimeoutAndFloors) {
    InMemoryGlobalConfig gc;
...
TEST(ContextualConfigTest, ResolveTimeoutAbsentKeepsFrozenDefault) {
    InMemoryGlobalConfig gc;
    EXPECT_EQ(ResolveContextualConfig(&gc).timeout_ms, kContextualTimeoutMs);
}

TEST(ContextualConfigTest, ResolveReadsGuardCharsPerTokenAndClamps) {
    InMemoryGlobalConfig gc;
...
TEST(ContextualConfigTest, ResolveGuardAbsentKeepsFrozenDefault) {
    InMemoryGlobalConfig gc;
    EXPECT_EQ(ResolveContextualConfig(&gc).guard_chars_per_token,
...
TEST(ContextualEnricherTest, GeneratePassesConfiguredTimeout) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    llm::LlmCallConfig captured;
    EXPECT_CALL(*llm, Chat(_, _))
        .WillOnce([&](const std::string&, const llm::LlmCallConfig& c) {
...
TEST(ContextualConfigTest, ResolveMissingKeysKeepDefaults) {
...
TEST(ContextualConfigTest, MergeNsOverrideEmptyJsonIsNoOp) {
...
TEST(ContextualConfigTest, MergeNsOverrideFlatKeys) {
    ContextualRetrievalConfig base;
...
TEST(ContextualConfigTest, MergeNsOverrideNestedUnderContextualRetrieval) {
    ContextualRetrievalConfig base;
    ContextualRetrievalConfig merged = MergeNsOverride(
...
TEST(ContextualConfigTest, MergeNsOverrideClampsAndIgnoresMalformed) {
    ContextualRetrievalConfig base;
...
TEST(ContextualConfigTest, MergeNsOverrideMalformedJsonIsNoOp) {
    ContextualRetrievalConfig base;
...
TEST(ContextualConfigTest, MergeNsOverrideEmptyModelIgnored) {
    ContextualRetrievalConfig base;
    base.llm_model = "gpt-4o";
...
TEST(ContextualOnnxEmbedderTest, NonNullEmbedderPropagatesStubEmbedding) {
...
TEST(ContextualOnnxEmbedderTest, NonNullEmbedderPropagatesFailure) {
...
TEST(ContextualConfigTest, MergeNsOverrideWrongTypesAllIgnored) {
    ContextualRetrievalConfig base;
    base.enabled = true;
...
TEST(ContextualConfigTest, MergeNsOverrideEmptyLlmModelIgnored) {
    ContextualRetrievalConfig base;
...
TEST(ContextualConfigTest, MergeNsOverrideNestedNotObjectFallsBackToFlat) {
    ContextualRetrievalConfig base;
    base.max_output_tokens = 80;
...
TEST(ContextualConfigTest, MergeNsOverrideNonObjectRootIsNoOp) {
    ContextualRetrievalConfig base;
...
TEST(ContextualEnricherTest, BuildPromptCustomTemplateSubstitutesMarkers) {
    ContextualRetrievalConfig cfg;
    cfg.prompt_template =
