...
// S1 coverage: ConnectionManager lifecycle — register / resolve /
// list / revoke + AES secret round-trip + 30d expiry + cross-tenant guard. Runs
// against the in-memory secret store + connection store (standalone doubles; the
// SQLite-backed store + real DSN resolution is integration).
