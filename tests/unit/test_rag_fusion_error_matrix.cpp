#include <gtest/gtest.h>

#include <regex>
#include <set>
#include <string>
#include <vector>

#include "cortrix/agent_friendly/error.h"
#include "cortrix/query/rag_fusion_error.h"

// Exhaustive parameterized error-registry sweep for RAG-Fusion (codes =
// 5 err + 1 warn). kDegraded uses the CX_WARN_ prefix, so the code-string regex
// accepts CX_ERR_ or CX_WARN_.
namespace cortrix::query {
namespace {

using agent_friendly::ErrorCategory;

const RagFusionErrorCode kAll[] = {
    RagFusionErrorCode::kDegraded,
    RagFusionErrorCode::kLlmTimeout,
    RagFusionErrorCode::kLlmQuota,
    RagFusionErrorCode::kLlmCircuitOpen,
    RagFusionErrorCode::kInvalidResponse,
    RagFusionErrorCode::kConfigInvalid,
};

bool IsValidCategory(ErrorCategory c) {
    return c == ErrorCategory::kAuth || c == ErrorCategory::kQuota ||
           c == ErrorCategory::kTransient || c == ErrorCategory::kPermanent ||
           c == ErrorCategory::kTimeout;
}

class RagFusionErrorMatrix : public ::testing::TestWithParam<RagFusionErrorCode> {};

TEST_P(RagFusionErrorMatrix, CodeStringFormat) {
    const RagFusionErrorCode code = GetParam();
    const std::string s = RagFusionErrorCodeString(code);
    EXPECT_FALSE(s.empty());
    EXPECT_TRUE(std::regex_match(s, std::regex("^CX_(ERR|WARN)_[A-Z0-9_]+$"))) << s;
    EXPECT_STREQ(GetRagFusionErrorInfo(code).cx_code, s.c_str());
}

TEST_P(RagFusionErrorMatrix, CategoryAndRetryInvariant) {
    const RagFusionErrorCode code = GetParam();
    const RagFusionErrorInfo& info = GetRagFusionErrorInfo(code);
    EXPECT_TRUE(IsValidCategory(info.category));
    if (!info.retryable) {
        EXPECT_FALSE(info.retry_after_ms.has_value()) << info.cx_code;
    }
    if (info.retry_after_ms.has_value()) {
        EXPECT_GT(*info.retry_after_ms, 0) << info.cx_code;
    }
}

TEST_P(RagFusionErrorMatrix, RequiredStructuredDataArms) {
    const RagFusionErrorCode code = GetParam();
    const std::vector<std::string>& keys = RequiredStructuredDataKeys(code);

    nlohmann::json full = nlohmann::json::object();
    for (const std::string& k : keys) full[k] = "v";
    EXPECT_TRUE(HasRequiredStructuredData(code, full));

    if (!keys.empty()) {
        nlohmann::json missing = full;
        missing.erase(keys.front());
        EXPECT_FALSE(HasRequiredStructuredData(code, missing));
    }

    EXPECT_EQ(HasRequiredStructuredData(code, nlohmann::json("not-an-object")),
              keys.empty());
}

TEST_P(RagFusionErrorMatrix, MakeRoundTripAndJson) {
    const RagFusionErrorCode code = GetParam();
    const RagFusionErrorInfo& info = GetRagFusionErrorInfo(code);

    nlohmann::json sd = nlohmann::json::object();
    for (const std::string& k : RequiredStructuredDataKeys(code)) sd[k] = "v";

    auto err = MakeRagFusionError(code, sd, "human detail");
    EXPECT_EQ(err.code, info.cx_code);
    EXPECT_EQ(err.message, "human detail");
    EXPECT_EQ(err.retryable, info.retryable);
    EXPECT_EQ(err.category, info.category);
    EXPECT_EQ(err.retry_after_ms, info.retry_after_ms);

    nlohmann::json j = agent_friendly::ToJson(err);
    EXPECT_EQ(j["code"], std::string(info.cx_code));
    EXPECT_EQ(j["retryable"], info.retryable);
    EXPECT_EQ(j["category"], std::string(agent_friendly::ToString(info.category)));
    EXPECT_EQ(j["retry_after_ms"].is_null(), !info.retry_after_ms.has_value());
}

TEST_P(RagFusionErrorMatrix, EmptyMessageFallsBackToCode) {
    const RagFusionErrorCode code = GetParam();
    auto err = MakeRagFusionError(code);
    EXPECT_EQ(err.message, std::string(RagFusionErrorCodeString(code)));
}

TEST_P(RagFusionErrorMatrix, StatusBridge) {
    const RagFusionErrorCode code = GetParam();
    const StatusCode mapped = RagFusionErrorToStatusCode(code);
    EXPECT_NE(mapped, StatusCode::kOk);

    Status s = RagFusionStatus(code, "detail-x");
    EXPECT_FALSE(s.ok());
    EXPECT_EQ(s.code(), mapped);
    EXPECT_NE(s.message().find(RagFusionErrorCodeString(code)), std::string::npos);
    EXPECT_NE(s.message().find("detail-x"), std::string::npos);
}

TEST(RagFusionErrorMatrixAggregate, CountAndUniqueCodes) {
    EXPECT_EQ(kRagFusionErrorCodeCount, 6);
    EXPECT_EQ(std::size(kAll), static_cast<size_t>(kRagFusionErrorCodeCount));
    std::set<std::string> seen;
    for (RagFusionErrorCode c : kAll) {
        EXPECT_TRUE(seen.insert(RagFusionErrorCodeString(c)).second);
    }
    EXPECT_EQ(seen.size(), static_cast<size_t>(kRagFusionErrorCodeCount));
}

INSTANTIATE_TEST_SUITE_P(AllCodes, RagFusionErrorMatrix, ::testing::ValuesIn(kAll));

}  // namespace
}  // namespace cortrix::query
