...
TEST(F40SparseFallbackTest, L1SerializesNonEmptyAndSetsFlag) {
...
TEST(F40SparseFallbackTest, L1EmptyVectorWritesNullClearsFlagNoError) {
...
TEST(F40SparseFallbackTest, L1OutOfRangeIdDegradesAfterRetries) {
...
TEST(F40SparseFallbackTest, L1RetriesCountConfigurable) {
...
TEST(F40SparseFallbackTest, L1RoundTripBlobIsValid) {
...
TEST(F40SparseFallbackTest, L2NormalPathWhenAvailable) {
...
TEST(F40SparseFallbackTest, L2FallbackWhenUnavailable) {
...
TEST(F40SparseFallbackTest, DropSparsePathZeroesSparseOnly) {
    FivePathInput in;
    in.dense = L({"a", "b"});
    in.sparse = L({"a"});
    in.fts5 = L({"b"});
...
TEST(F40SparseFallbackTest, L2FusionDegradesToFourPath) {
