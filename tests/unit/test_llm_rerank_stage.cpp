...
TEST(LlmRerankExplainTest, ActiveBlockCarriesTheCallCountersAndNoTrackingId) {
    auto mock = std::make_shared<MockLlmClient>();
    EXPECT_CALL(*mock, Chat(_, _))
    ...
    // No internal tracking identifier is emitted.
    EXPECT_FALSE(lr.contains("feature_id"));
}
