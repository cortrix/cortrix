...
TEST(SparseErrorTest, CountIsFive) {
    EXPECT_EQ(kSparseErrorCodeCount, 5);
    EXPECT_EQ(std::size(kAll), 5u);
}

TEST(SparseErrorTest, CodeStringsMatchArchRegistry) {
...
TEST(SparseErrorTest, AllCodesUniqueAndPrefixed) {
...
TEST(SparseErrorTest, RetryableImpliesRetryAfterSet) {
    for (auto code : kAll) {
...
TEST(SparseErrorTest, RequiredStructuredDataKeys) {
...
TEST(SparseErrorTest, HasRequiredStructuredData) {
...
TEST(SparseErrorTest, MakeSparseErrorFillsFromRegistry) {
...
TEST(SparseErrorTest, MakeSparseErrorDefaultMessageIsCode) {
...
TEST(SparseErrorTest, MakeSparseErrorSerializesToAgentFriendlyBody) {
...
TEST(SparseErrorTest, StatusBridgeCarriesTokenAndCode) {
...
TEST(SparseErrorTest, StatusCodeMapping) {
