// DocSummaryAsyncWorker — the async task doc_summary task callback. These are
...
class DocSummaryAsyncWorkerTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(DocSummaryAsyncWorkerTest, WritesDocSummaryBlockOnSuccess) {
...
TEST_F(DocSummaryAsyncWorkerTest, EmptyDocumentCompletesWithoutWritingBlock) {
...
TEST_F(DocSummaryAsyncWorkerTest, InvalidLlmOutputWritesNoBlock) {
    const std::string doc_id = SeedDoc(2);
    DocSummaryAsyncWorker worker = MakeWorker(MakeLlm("not valid json"));
...
TEST_F(DocSummaryAsyncWorkerTest, AcquireFailureOnUnknownNamespaceReturnsError) {
    DocSummaryAsyncWorker worker = MakeWorker(MakeLlm(kSummaryJson));
