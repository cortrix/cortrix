...
TEST(F40SparseRetrieverInterfaceTest, UsableViaBasePointer) {
    InMemorySparseRetriever impl;
    ISparseRetriever* r = &impl;  // polymorphic use
    EXPECT_TRUE(r->IsAvailable());
}

TEST(F40SparseRetrieverInterfaceTest, AddThenSearchReturnsHit) {
...
TEST(F40SparseRetrieverInterfaceTest, EmptyNamespaceReturnsEmpty) {
    InMemorySparseRetriever r;
    auto hits = r.Search(V({{1, 1.0f}}), "no_such_ns", 10);
    EXPECT_TRUE(hits.empty());  // empty NS = success, not error
}

TEST(F40SparseRetrieverInterfaceTest, SearchRanksByScoreAndTruncates) {
    InMemorySparseRetriever r;
...
TEST(F40SparseRetrieverInterfaceTest, AddEmptyVectorRemoves) {
    InMemorySparseRetriever r;
    r.Add("ns", "c", V({{1, 0.5f}}));
...
TEST(F40SparseRetrieverInterfaceTest, RemoveIsIdempotent) {
    InMemorySparseRetriever r;
...
TEST(F40SparseRetrieverInterfaceTest, ReAddReplacesPostings) {
    InMemorySparseRetriever r;
    r.Add("ns", "c", V({{1, 0.5f}}));
...
TEST(F40SparseRetrieverInterfaceTest, NamespaceIsolation) {
    InMemorySparseRetriever r;
...
TEST(F40SparseRetrieverInterfaceTest, IsAvailableReflectsBackingState) {
    InMemorySparseRetriever r;
    EXPECT_TRUE(r.IsAvailable());
