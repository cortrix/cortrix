...
class DocSummaryFts5IndexTest : public ::testing::Test {
protected:
    void SetUp() override { ASSERT_TRUE(idx_.Open(":memory:").ok()); }
    DocFts5Index idx_;
};

TEST_F(DocSummaryFts5IndexTest, OpenCreatesUsableIndex) {
    EXPECT_TRUE(idx_.is_open());
}

TEST_F(DocSummaryFts5IndexTest, UpsertThenSearchMatchesTitle) {
...
TEST_F(DocSummaryFts5IndexTest, SearchMatchesTopics) {
...
TEST_F(DocSummaryFts5IndexTest, MultiTokenQueryMatchesPartialTerms) {
...
TEST_F(DocSummaryFts5IndexTest, UpsertIsIdempotentOnDocId) {
...
TEST_F(DocSummaryFts5IndexTest, EmptyQueryReturnsNoResults) {
...
TEST_F(DocSummaryFts5IndexTest, ZeroTopKReturnsEmpty) {
    ASSERT_TRUE(idx_.Upsert(Row("d1", "Something")).ok());
...
TEST_F(DocSummaryFts5IndexTest, Fts5OperatorInjectionIsSanitized) {
...
TEST(DocSummaryFts5IndexLifecycleTest, SearchOnClosedIndexErrors) {
...
TEST(DocSummaryFts5IndexLifecycleTest, UpsertOnClosedIndexErrors) {
    DocFts5Index idx;
    auto st = idx.Upsert(Row("d1", "t"));
    EXPECT_FALSE(st.ok());
}

TEST(DocSummaryFts5IndexLifecycleTest, OpenOnUncreatablePathErrors) {
...
TEST(DocSummaryFts5SharedHandleTest, BorrowedHandleUpsertSearchDelete) {
    sqlite3* db = nullptr;
...
TEST(DocSummaryFts5SharedHandleTest, BorrowedHandleErrorsWithoutMigratedTable) {
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
...
TEST(DocSummaryFuseDocDiscoveryTest, MergesBothPathsAndDedups) {
...
TEST(DocSummaryFuseDocDiscoveryTest, Fts5OnlyDocGetsFts5ViaPath) {
...
TEST(DocSummaryFuseDocDiscoveryTest, TopKTruncates) {
    std::vector<DocDiscoveryHit> llm = {LlmHit("d1", "s1"), LlmHit("d2", "s2"),
                                        LlmHit("d3", "s3")};
...
TEST(DocSummaryFuseDocDiscoveryTest, RankOrderPreservedByRrf) {
...
TEST(DocSummaryFuseDocDiscoveryTest, EmptyInputsGiveEmpty) {
