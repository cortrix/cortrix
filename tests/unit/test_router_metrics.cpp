...
TEST_F(RouterMetricsTest, DecisionToString) {
...
TEST_F(RouterMetricsTest, SavedPathToString) {
    EXPECT_STREQ(ToString(SavedPath::kSimple), "simple");
    EXPECT_STREQ(ToString(SavedPath::kChat), "chat");
}

TEST_F(RouterMetricsTest, DecisionCounterPerLabel) {
...
TEST_F(RouterMetricsTest, ClassifierLatencyHistogram) {
    auto& m = QueryRouterMetrics::Instance();
...
TEST_F(RouterMetricsTest, NegativeLatencyClampedToZero) {
    auto& m = QueryRouterMetrics::Instance();
...
TEST_F(RouterMetricsTest, FallbackRatioGaugeLastWriteWins) {
    auto& m = QueryRouterMetrics::Instance();
...
TEST_F(RouterMetricsTest, ComputeSavedAccumulatesPerPath) {
    auto& m = QueryRouterMetrics::Instance();
...
TEST_F(RouterMetricsTest, ComputeSavedNegativeClampedToZero) {
    auto& m = QueryRouterMetrics::Instance();
    m.AddComputeSaved(SavedPath::kSimple, -1.0);
    EXPECT_NEAR(m.ComputeSavedSeconds(SavedPath::kSimple), 0.0, 1e-6);
}

TEST_F(RouterMetricsTest, RenderOpenMetricsContainsAll4Metrics) {
    auto& m = QueryRouterMetrics::Instance();
    m.RecordDecision(Decision::kSimple);
    m.RecordDecision(Decision::kComplex);
...
TEST_F(RouterMetricsTest, ResetForTestClearsEverything) {
    auto& m = QueryRouterMetrics::Instance();
    m.RecordDecision(Decision::kSimple);
