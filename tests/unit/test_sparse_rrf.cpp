...
TEST(SparseRrfTest, SinglePathRrfScore) {
    FivePathInput in;
    in.sparse = L({"a", "b", "c"});
...
TEST(SparseRrfTest, ChildInMultiplePathsAccumulates) {
    FivePathInput in;
...
TEST(SparseRrfTest, ContributingPathsRecorded) {
    FivePathInput in;
    in.dense = L({"a"});
    in.sparse = L({"a"});
    in.fts5 = L({"b"});
...
TEST(SparseRrfTest, MockContextualizedAndHypeEmptyAreNoOps) {
    FivePathInput in;
    in.dense = L({"a"});
    in.sparse = L({"a"});
...
TEST(SparseRrfTest, MockPathsCanContributeWhenPopulated) {
...
TEST(SparseRrfTest, DedupAcrossPaths) {
    FivePathInput in;
    in.dense = L({"a", "b"});
    in.sparse = L({"a", "b"});
    auto out = FuseFivePathRrf(in);
...
TEST(SparseRrfTest, TopNTruncates) {
    FivePathInput in;
...
TEST(SparseRrfTest, TopNZeroKeepsAll) {
    FivePathInput in;
    in.dense = L({"a", "b", "c"});
    EXPECT_EQ(FuseFivePathRrf(in, 0).size(), 3u);
}

TEST(SparseRrfTest, SortedDescendingWithDeterministicTieBreak) {
    FivePathInput in;
...
TEST(SparseRrfTest, EmptyInputEmptyOutput) {
    FivePathInput in;
    EXPECT_TRUE(FuseFivePathRrf(in).empty());
}

TEST(SparseRrfTest, KZeroGuardedToDefault) {
    FivePathInput in;
    in.dense = L({"a"});
