...
TEST_F(ImportSchemaTest, MetadataMatchesFrozenInterface) {
    ImportSchemaProvider p;
    EXPECT_EQ(p.FeatureName(), "db_import");
    EXPECT_EQ(p.CurrentVersion(), 1);
    EXPECT_EQ(kImportSchemaVersion, 1);
}

TEST_F(ImportSchemaTest, MigrateCreatesTablesAndIndices) {
    ImportSchemaProvider p;
...
TEST_F(ImportSchemaTest, MigrateIsIdempotent) {
    ImportSchemaProvider p;
...
TEST_F(ImportSchemaTest, UnsupportedStepIsVersionMismatch) {
    ImportSchemaProvider p;
...
TEST_F(ImportSchemaTest, ForeignKeysAndColumnsUsable) {
    ImportSchemaProvider p;
    ASSERT_TRUE(p.Migrate(db_, 0, 1).ok());
