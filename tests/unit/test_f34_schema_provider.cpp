...
TEST(F34SchemaProviderTest, AddsChildColumnsIndexesAndParents) {
    sqlite3* db = nullptr;
...
TEST(F34SchemaProviderTest, ChildIdUniqueButNullsAllowed) {
...
TEST(F34SchemaProviderTest, MetaDocUniquePerDoc) {
...
TEST(F34SchemaProviderTest, MigrationIdempotent) {
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
    CreateBlocksTable(db);
    F34SchemaProvider p;
...
TEST(F34SchemaProviderTest, NoBlocksTableStillCreatesParents) {
...
TEST(F34SchemaProviderTest, UnexpectedVersionStepIsError) {
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
    F34SchemaProvider p;
...
TEST(F34SchemaProviderTest, NullDbRejected) {
...
TEST(F34SchemaProviderTest, FromZeroToUnsupportedTargetIsError) {
...
TEST(F34SchemaProviderTest, SameVersionNoOpSucceeds) {
...
TEST(F34SchemaProviderTest, ReadOnlyDbFailsParentsExec) {
...
TEST(F34SchemaProviderTest, IndexNameCollisionFailsExec) {
...
TEST(F34SchemaProviderTest, ParentIndexNameCollisionFailsExec) {
...
TEST(F34SchemaProviderTest, MetaDocIndexNameCollisionFailsExec) {
