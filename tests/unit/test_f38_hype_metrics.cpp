...
class F38HypeMetricsTest : public ::testing::Test {
protected:
...
TEST_F(F38HypeMetricsTest, LlmCallsCounterPerStatus) {
...
TEST_F(F38HypeMetricsTest, QuestionsGeneratedCounter) {
    M().AddQuestionsGenerated(3);
    M().AddQuestionsGenerated(3);
    EXPECT_EQ(M().QuestionsGeneratedCount(), 6u);
}

TEST_F(F38HypeMetricsTest, MatchCounterPerHitType) {
...
TEST_F(F38HypeMetricsTest, LlmDurationHistogramPerModel) {
...
TEST_F(F38HypeMetricsTest, LabelStrings) {
...
TEST_F(F38HypeMetricsTest, RenderContainsAll4Metrics) {
    M().RecordLlmCall(LS::kSuccess);
    M().AddQuestionsGenerated(3);
    M().RecordMatch(HT::kBoth);
...
TEST_F(F38HypeMetricsTest, NoForbiddenHighCardinalityLabels) {
    M().RecordLlmCall(LS::kSuccess);
    M().RecordMatch(HT::kChunk);
    M().ObserveLlmDuration("gpt-4o-mini", 0.5);
    std::string out = M().RenderOpenMetrics();
...
TEST_F(F38HypeMetricsTest, ResetForTestClearsAll) {
    M().RecordLlmCall(LS::kSuccess);
