// HyPE S0 — kBlockHypeQuestion=16 block sub-type + HypeSchemaProvider. Unlike enricher
...
TEST(HypeBlockTypeTest, HypeQuestionIs16) {
...
TEST(HypeBlockTypeTest, DoesNotCollideWithExistingTypes) {
...
TEST(HypeSchemaProviderTest, IdentityAndVersion) {
    HypeSchemaProvider p;
    EXPECT_EQ(p.FeatureName(), "hype");
    EXPECT_EQ(p.CurrentVersion(), 1);
}

TEST(HypeSchemaProviderTest, MigrateIsNoOpNoNewTable) {
    HypeSchemaProvider p;
    sqlite3* db = nullptr;
...
TEST(HypeSchemaProviderTest, UnexpectedVersionStepIsError) {
    HypeSchemaProvider p;
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
...
TEST(HypeSchemaProviderTest, RegistersWithSchemaMigrator) {
