...
#include "cortrix/agent_trace/agent_trace_cleanup_registrar.h"
...
class AgentTraceCleanupRegistrarTest : public ::testing::Test {
protected:
    void SetUp() override {
        ...
        registrar_ = std::make_unique<AgentTraceCleanupRegistrar>(writer_, db_, config_);
    }
    ...
    std::unique_ptr<AgentTraceCleanupRegistrar> registrar_;
    sqlite3* db_ = nullptr;
};

TEST_F(AgentTraceCleanupRegistrarTest, RegistersAgentTraceOnly) {
...
TEST_F(AgentTraceCleanupRegistrarTest, RunCleanupNowPrunesAgentTrace) {
...
TEST_F(AgentTraceCleanupRegistrarTest, CleanupInteractionLogPrunesAndCascades) {
    ...
    const int64_t now = NowMs();
    const std::string old_iso = AgentTraceCleanupRegistrar::FormatIso8601Utc(now - 181LL * 86400000LL);
    const std::string new_iso = AgentTraceCleanupRegistrar::FormatIso8601Utc(now - 1000);
...
TEST_F(AgentTraceCleanupRegistrarTest, CleanupInteractionLogReturnsDeletedCount) {
    const int64_t now = NowMs();
    const std::string old_iso = AgentTraceCleanupRegistrar::FormatIso8601Utc(now - 200LL * 86400000LL);
    Exec(db_, "INSERT INTO interaction_log(id, session_id, namespace_name, user_id, role, content, created_at) "
...
TEST_F(AgentTraceCleanupRegistrarTest, FormatIso8601UtcShape) {
    // 2026-06-01T00:00:00.000Z is a known epoch-ms; just assert the shape + 'Z'.
    std::string s = AgentTraceCleanupRegistrar::FormatIso8601Utc(1764547200123LL);
...
TEST_F(AgentTraceCleanupRegistrarTest, NullDbInteractionLogCleanupIsZero) {
    AgentTraceCleanupRegistrar no_db(writer_, nullptr, config_);
