// Sparse retrieval S5 — SparseSchemaProvider creates the net-new sparse_inverted_index table
...
TEST(SparseSchemaProviderTest, IdentityAndVersion) {
    SparseSchemaProvider p;
    EXPECT_EQ(p.FeatureName(), "sparse_index");
    EXPECT_EQ(p.CurrentVersion(), 1);
}

TEST(SparseSchemaProviderTest, MigrateCreatesTableAndIndexes) {
    SparseSchemaProvider p;
    sqlite3* db = nullptr;
...
TEST(SparseSchemaProviderTest, MigrateAddsSparseVecToBlocks) {
...
TEST(SparseSchemaProviderTest, MigrateIsIdempotent) {
    SparseSchemaProvider p;
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
    EXPECT_TRUE(p.Migrate(db, 0, 1).ok());
...
TEST(SparseSchemaProviderTest, UnexpectedVersionStepIsError) {
    SparseSchemaProvider p;
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
...
TEST(SparseSchemaProviderTest, NullDbRejected) {
    SparseSchemaProvider p;
    Status st = p.Migrate(nullptr, 0, 1);
    EXPECT_FALSE(st.ok());
}

TEST(SparseSchemaProviderTest, RegistersWithSchemaMigrator) {
