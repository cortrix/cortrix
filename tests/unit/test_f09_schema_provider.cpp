...
TEST(F09SchemaProviderTest, MigrateBuildsFrameworkSchema) {
    F09SchemaProvider p;
    sqlite3* db = nullptr;
...
TEST(F09SchemaProviderTest, FtsRoundtripAndTriggers) {
    F09SchemaProvider p;
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
    ASSERT_TRUE(p.Migrate(db, 0, 1).ok());
...
TEST(F09SchemaProviderTest, BlockIdIsRowidAliasNotAutoincrement) {
    F09SchemaProvider p;
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
    ASSERT_TRUE(p.Migrate(db, 0, 1).ok());
...
TEST(F09SchemaProviderTest, UnexpectedVersionStepIsError) {
    F09SchemaProvider p;
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
...
TEST(F09SchemaProviderTest, FrameworkDdlExecFailureReported) {
...
TEST(F09SchemaProviderTest, FromZeroToUnsupportedTargetIsError) {
