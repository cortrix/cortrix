...
TEST(F40SpladeQueryTest, SingleTermScoreIsProduct) {
    auto r = Open();
...
TEST(F40SpladeQueryTest, MultiTermScoreAccumulates) {
    auto r = Open();
...
TEST(F40SpladeQueryTest, NoSharedTermsNoHit) {
    auto r = Open();
...
TEST(F40SpladeQueryTest, RanksMultipleChildren) {
    auto r = Open();
...
TEST(F40SpladeQueryTest, ChildScoredAcrossMultiplePostings) {
...
TEST(F40SpladeQueryTest, TopKTruncates) {
    auto r = Open();
    for (int i = 0; i < 10; ++i) {
...
TEST(F40SpladeQueryTest, TopKZeroOrNegativeUsesDefault) {
    SpladeConfig cfg;
    cfg.default_top_k = 2;
    auto r = Open(cfg);
    for (int i = 0; i < 5; ++i) {
...
TEST(F40SpladeQueryTest, EmptyQueryReturnsEmpty) {
    auto r = Open();
    r->Add("ns", "c", V({{1, 0.5f}}));
    EXPECT_TRUE(r->Search(V({}), "ns", 10).empty());
}

TEST(F40SpladeQueryTest, NotOpenSearchFlagsFallback) {
...
TEST(F40SpladeQueryTest, OpenSearchDoesNotFlagFallback) {
    auto r = Open();
    r->Add("ns", "c", V({{1, 0.5f}}));
...
TEST(F40SpladeQueryTest, PostingListCapLimitsScan) {
    SpladeConfig cfg;
...
TEST(F40SpladeQueryTest, RepeatedQueryHitsCache) {
    auto r = Open();
...
TEST(F40SpladeQueryTest, AddInvalidatesCachedPostingList) {
    auto r = Open();
...
TEST(F40SpladeQueryTest, RemoveInvalidatesCachedPostingList) {
    auto r = Open();
    r->Add("ns", "c1", V({{1, 0.5f}}));
    r->Add("ns", "c2", V({{1, 0.8f}}));
...
TEST(F40SpladeQueryTest, CacheEvictsBeyondCapacity) {
    SpladeConfig cfg;
