...
class F39ClassifierTest : public ::testing::Test {
protected:
...
TEST_F(F39ClassifierTest, IClassifierIdentity) {
...
TEST_F(F39ClassifierTest, IsAvailableFalseWhenBackendNull) {
    QueryComplexityClassifier c(nullptr, DefaultConfig());
    EXPECT_FALSE(c.IsAvailable());
}

TEST_F(F39ClassifierTest, IsAvailableFalseWhenBackendUnavailable) {
...
TEST_F(F39ClassifierTest, ClassifyReturnsBackendLabelAndRecordsLatency) {
    auto backend = std::make_shared<MockComplexityBackend>();
...
TEST_F(F39ClassifierTest, ClassifyDegradesToComplexWhenUnavailable) {
    QueryComplexityClassifier c(nullptr, DefaultConfig());
...
TEST_F(F39ClassifierTest, ForceRouteOverridesToSimple) {
    auto backend = std::make_shared<MockComplexityBackend>();
    QueryComplexityClassifier c(backend, DefaultConfig());
    QueryContext ctx;
...
TEST_F(F39ClassifierTest, ForceRouteChatSetsChatFlag) {
    auto backend = std::make_shared<MockComplexityBackend>();
    QueryComplexityClassifier c(backend, DefaultConfig());
    QueryContext ctx;
...
TEST_F(F39ClassifierTest, ForceRouteAutoFallsThroughToClassifier) {
    auto backend = std::make_shared<MockComplexityBackend>();
...
TEST_F(F39ClassifierTest, ForceRouteInvalidReturnsErrorAndLeavesCtxUntouched) {
    auto backend = std::make_shared<MockComplexityBackend>();
    QueryComplexityClassifier c(backend, DefaultConfig());
    QueryContext ctx;
...
TEST_F(F39ClassifierTest, NsForceRouteOverrides) {
    auto backend = std::make_shared<MockComplexityBackend>();
...
TEST_F(F39ClassifierTest, NsForceRouteInvalidReturnsError) {
    auto backend = std::make_shared<MockComplexityBackend>();
    ComplexityConfig cfg = DefaultConfig();
...
TEST_F(F39ClassifierTest, ChatRuleGuardRoutesGreeting) {
    auto backend = std::make_shared<MockComplexityBackend>();
    QueryComplexityClassifier c(backend, DefaultConfig());
    QueryContext ctx;
    ctx.query = "Hello!";
...
TEST_F(F39ClassifierTest, IsChatQueryRecognizesPleasantries) {
...
TEST_F(F39ClassifierTest, NsDisabledDefaultsToComplex) {
    auto backend = std::make_shared<MockComplexityBackend>();
    ComplexityConfig cfg = DefaultConfig();
    cfg.enabled = false;
    QueryComplexityClassifier c(backend, cfg);
...
TEST_F(F39ClassifierTest, BackendUnavailableDefaultsToComplex) {
    QueryComplexityClassifier c(nullptr, DefaultConfig());
    QueryContext ctx;
    ctx.query = "find the onboarding guide";
    Status s = c.RouteAndUpdateContext(ctx);
...
TEST_F(F39ClassifierTest, ClassifierSimpleVerdictWritesLlmSource) {
    auto backend = std::make_shared<MockComplexityBackend>();
    backend->label = "simple";
...
TEST_F(F39ClassifierTest, ClassifierChatVerdictIsDemotedToComplex) {
...
TEST_F(F39ClassifierTest, NsThresholdClampedToUpperBound) {
    auto backend = std::make_shared<MockComplexityBackend>();
    backend->label = "simple";
...
TEST_F(F39ClassifierTest, NsThresholdClampedToLowerBound) {
    auto backend = std::make_shared<MockComplexityBackend>();
    backend->label = "simple";
...
TEST_F(F39ClassifierTest, TransientFaultRetriedThenSucceeds) {
    auto backend = std::make_shared<MockComplexityBackend>();
...
TEST_F(F39ClassifierTest, AllRetriesFailDegradeToComplex) {
    auto backend = std::make_shared<MockComplexityBackend>();
...
TEST_F(F39ClassifierTest, ClassifyTotalOnPersistentFault) {
...
TEST_F(F39ClassifierTest, MultiTurnSignalSetsWarning) {
    auto backend = std::make_shared<MockComplexityBackend>();
    backend->label = "complex";
    backend->confidence = 0.9f;
    QueryComplexityClassifier c(backend, DefaultConfig());
...
TEST_F(F39ClassifierTest, MultiTurnWarningDisabledByConfig) {
    auto backend = std::make_shared<MockComplexityBackend>();
    backend->label = "complex";
    backend->confidence = 0.9f;
    ComplexityConfig cfg = DefaultConfig();
...
TEST_F(F39ClassifierTest, HasMultiTurnSignalDetectsCuesAndAvoidsFalsePositives) {
...
TEST_F(F39ClassifierTest, ChatFlagAloneTriggersSkipEvenWithoutRoutingPath) {
...
TEST_F(F39ClassifierTest, HeuristicBackendRoutesComparisonToComplex) {
    auto backend = std::make_shared<HeuristicComplexityBackend>();
    QueryComplexityClassifier c(backend, DefaultConfig());
    QueryContext ctx;
...
TEST_F(F39ClassifierTest, HeuristicBackendRoutesShortQueryToSimple) {
    auto backend = std::make_shared<HeuristicComplexityBackend>();
    QueryComplexityClassifier c(backend, DefaultConfig());
    QueryContext ctx;
...
TEST_F(F39ClassifierTest, ConfidentComplexCountsAsComplexNotFallback) {
    auto backend = std::make_shared<MockComplexityBackend>();
    backend->label = "complex";
...
TEST_F(F39ClassifierTest, ClassifierUnavailableCountsAsFallback) {
    QueryComplexityClassifier c(nullptr, DefaultConfig());
    QueryContext ctx;
    ctx.query = "find the onboarding guide";
    Status s = c.RouteAndUpdateContext(ctx);
...
TEST_F(F39ClassifierTest, SimpleVerdictCountsAsSimple) {
    auto backend = std::make_shared<MockComplexityBackend>();
    backend->label = "simple";
    backend->confidence = 0.9f;
    QueryComplexityClassifier c(backend, DefaultConfig());
