...
using QueueState = F42Metrics::QueueState;
using CancelPhase = F42Metrics::CancelPhase;

class F42MetricsTest : public ::testing::Test {
protected:
...
TEST_F(F42MetricsTest, SubmittedCountsByTaskType) {
...
TEST_F(F42MetricsTest, CompletedCountsByTaskTypeAndStatus) {
...
TEST_F(F42MetricsTest, QueueDepthGaugeByStateClamped) {
...
TEST_F(F42MetricsTest, ZombieCleanedAndCancelCounters) {
...
TEST_F(F42MetricsTest, RenderOpenMetricsHasAllSixMetricsAndTypes) {
    M().RecordSubmitted(kTaskDocParse);
    M().RecordCompleted(kTaskDocParse, CompletionStatus::kSuccess);
...
TEST_F(F42MetricsTest, DurationHistogramBucketsAreCumulativeAndComplete) {
...
TEST_F(F42MetricsTest, RenderHasNoHighCardinalityLabels) {
...
TEST_F(F42MetricsTest, TaskTypeIndexFoldsUnknownIntoDocParse) {
...
TEST_F(F42MetricsTest, LabelStringsMatchSpec) {
