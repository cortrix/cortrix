...
TEST(F39RouterErrorTest, CountMatchesEnumeration) {
    EXPECT_EQ(kRouterErrorCodeCount, 4);
    EXPECT_EQ(std::size(kAll), static_cast<size_t>(kRouterErrorCodeCount));
}

TEST(F39RouterErrorTest, AllCodesHaveUniqueF39CxStrings) {
...
TEST(F39RouterErrorTest, RegistryMatchesSpecTable) {
...
TEST(F39RouterErrorTest, NonRetryablesCarryNoRetryHint) {
...
TEST(F39RouterErrorTest, RequiredStructuredDataKeysPerSpec) {
...
TEST(F39RouterErrorTest, HasRequiredStructuredDataValidatesKeys) {
...
TEST(F39RouterErrorTest, EveryCodeRequiresAtLeastOneStructuredKey) {
    for (RouterErrorCode c : kAll) {
        EXPECT_FALSE(RequiredStructuredDataKeys(c).empty()) << RouterErrorCodeString(c);
    }
}

TEST(F39RouterErrorTest, MakeRouterErrorFillsFromRegistry) {
...
TEST(F39RouterErrorTest, ToJsonSerializesAgentFriendlyBody) {
...
TEST(F39RouterErrorTest, StatusBridgeCarriesCodeToken) {
