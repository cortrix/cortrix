...
TEST(DocSummaryGeneratorTest, ParseValidJson) {
...
TEST(DocSummaryGeneratorTest, ParseNonObjectIsInvalidOutput) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
...
TEST(DocSummaryGeneratorTest, ParseGarbageIsInvalidOutput) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
...
TEST(DocSummaryGeneratorTest, ParseCompleteFencedJson) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
...
TEST(DocSummaryGeneratorTest, ParseRepairsInvalidBackslashEscapeInJsonString) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
...
TEST(DocSummaryGeneratorTest, ParseMissingSummaryTextIsInvalid) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
...
TEST(DocSummaryGeneratorTest, ParseRepairsProseWrappedJson) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
    std::string repair;
...
TEST(DocSummaryGeneratorTest, ParseRepairsProseWrappedJsonWithInvalidEscape) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
...
TEST(DocSummaryGeneratorTest, ParseRepairsUnclosedFence) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
    std::string repair;
    auto r = g.ParseStructuredOutput(std::string("```json\n") + kGoodJson, 500,
...
TEST(DocSummaryGeneratorTest, ParseRepairHonorsBracesInsideStrings) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
    std::string repair;
    auto r = g.ParseStructuredOutput(
...
TEST(DocSummaryGeneratorTest, ParseTruncatedJsonStillFails) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
    std::string repair;
    auto r = g.ParseStructuredOutput(
...
TEST(DocSummaryGeneratorTest, ParseStrictPathSetsNoRepairMarker) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
...
TEST(DocSummaryGeneratorTest, ParseTruncatesSummaryToMaxChars) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
...
TEST(DocSummaryGeneratorTest, ParseTruncationIsUtf8Safe) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
...
TEST(DocSummaryGeneratorTest, BuildPromptHasStructuredRequirements) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
...
TEST(DocSummaryGeneratorTest, BuildGroupAndReducePrompts) {
    DocSummaryGenerator g = MakeGen(std::make_shared<llm::MockLlmClient>(),
                                    std::make_shared<store::MockChunkStore>());
...
TEST(DocSummaryGeneratorTest, ShortDocSingleCall) {
...
TEST(DocSummaryGeneratorTest, StructuredCallUsesDeepSeekJsonContract) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(DocSummaryGeneratorTest, StructuredCallKeepsGlmPromptOnlyContract) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    llm::LlmCallConfig captured;
    EXPECT_CALL(*llm, Chat(_, _))
        .WillOnce(DoAll(SaveArg<1>(&captured), Return(OkChat(kGoodJson))));
...
TEST(DocSummaryGeneratorTest, ShortDocLlmFailureIsTimeout) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(DocSummaryGeneratorTest, SendsConfiguredModelNotDefault) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    DocSummaryConfig cfg;
    cfg.llm_model = "glm-5.2";
...
TEST(DocSummaryGeneratorTest, MapReduceMapStageFailureSurfaces) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    DocSummaryConfig cfg;
...
TEST(DocSummaryGeneratorTest, GenerateSuccess) {
...
TEST(DocSummaryGeneratorTest, GenerateNoChunksCompletesWithoutSummaryContent) {
    DocSummaryMetrics::Instance().ResetForTest();
    auto llm = std::make_shared<llm::MockLlmClient>();
    auto store = std::make_shared<store::MockChunkStore>();
...
TEST(DocSummaryGeneratorTest, GenerateLlmFailureCarriesTimeoutError) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    llm::ChatCompletionResponse fail;
...
TEST(DocSummaryGeneratorTest, GenerateInvalidOutputCarriesInvalidError) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(DocSummaryGeneratorTest, GenerateInvalidOutputCapturesReasoningDiagnostics) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(DocSummaryGeneratorTest, GenerateNullChunkStoreErrors) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(DocSummaryConfigTest, ResolveReadsGlobalLayer) {
...
TEST(DocSummaryConfigTest, ResolveIgnoresNonPositiveAndUnparseable) {
    InMemoryGlobalConfig gc;
