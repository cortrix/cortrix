...
class F41DocSummaryMetricsTest : public ::testing::Test {
protected:
...
TEST_F(F41DocSummaryMetricsTest, LlmCallsCountPerStatus) {
...
TEST_F(F41DocSummaryMetricsTest, LlmDurationHistogramPerModel) {
...
TEST_F(F41DocSummaryMetricsTest, SummariesGeneratedCounter) {
    m().AddSummariesGenerated(1);
    m().AddSummariesGenerated(2);
    EXPECT_EQ(m().SummariesGeneratedCount(), 3u);
}

TEST_F(F41DocSummaryMetricsTest, FallbackTriggeredPerResult) {
...
TEST_F(F41DocSummaryMetricsTest, Fts5FallbackFailedCounter) {
    m().RecordFts5FallbackFailed();
    m().RecordFts5FallbackFailed();
    EXPECT_EQ(m().Fts5FallbackFailedCount(), 2u);
}

TEST_F(F41DocSummaryMetricsTest, LabelStrings) {
...
TEST_F(F41DocSummaryMetricsTest, RenderOpenMetricsContainsAllSeries) {
    m().RecordLlmCall(LlmCallStatus::kSuccess);
...
TEST_F(F41DocSummaryMetricsTest, RenderHasNoHighCardinalityLabels) {
    m().AddSummariesGenerated(1);
    std::string out = m().RenderOpenMetrics();
...
TEST_F(F41DocSummaryMetricsTest, ResetClearsEverything) {
    m().RecordLlmCall(LlmCallStatus::kSuccess);
