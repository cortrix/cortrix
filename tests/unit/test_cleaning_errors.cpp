...
TEST(CleaningErrorTest, RowsMatchSpec) {
    ...
    check(CleaningErrorCode::kDedupVectorInvalid, "CX_ERR_F10_DEDUP_VECTOR_INVALID",
          ErrorCategory::kPermanent, false, std::nullopt);
    check(CleaningErrorCode::kDedupThresholdRange, "CX_ERR_F10_DEDUP_THRESHOLD_RANGE",
          ErrorCategory::kPermanent, false, std::nullopt);
    check(CleaningErrorCode::kAnomalyConfigInvalid, "CX_ERR_F10_ANOMALY_CONFIG_INVALID",
          ErrorCategory::kPermanent, false, std::nullopt);
    check(CleaningErrorCode::kPluginTimeout, "CX_ERR_F10_PLUGIN_TIMEOUT",
          ErrorCategory::kTimeout, true, 1000);
    check(CleaningErrorCode::kPluginException, "CX_ERR_F10_PLUGIN_EXCEPTION",
          ErrorCategory::kPermanent, false, std::nullopt);
    check(CleaningErrorCode::kNsConfigMergeFailed, "CX_ERR_F10_NS_CONFIG_MERGE_FAILED",
          ErrorCategory::kPermanent, false, std::nullopt);
    check(CleaningErrorCode::kInternalError, "CX_ERR_F10_INTERNAL_ERROR",
...
TEST(CleaningErrorTest, MakeCleaningErrorBuildsAgentFriendlyBody) {
    auto err = MakeCleaningError(CleaningErrorCode::kPluginTimeout,
                                 ...
                                 "plugin too slow");
    EXPECT_EQ(err.code, "CX_ERR_F10_PLUGIN_TIMEOUT");
    ...
    EXPECT_EQ((*err.structured_data)["code"], "CX_ERR_F10_PLUGIN_TIMEOUT");
    EXPECT_EQ((*err.structured_data)["plugin"], "redactor");

    nlohmann::json body = agent_friendly::ToJson(err);
    EXPECT_EQ(body["code"], "CX_ERR_F10_PLUGIN_TIMEOUT");
