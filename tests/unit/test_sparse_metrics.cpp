...
class SparseMetricsTest : public ::testing::Test {
protected:
...
TEST_F(SparseMetricsTest, InferenceCounterPerStatus) {
...
TEST_F(SparseMetricsTest, FallbackRatioGauge) {
...
TEST_F(SparseMetricsTest, InvertedIndexSizeGauge) {
    M().SetInvertedIndexSizeBytes(602u * 1000u);
    EXPECT_EQ(M().InvertedIndexSizeBytes(), 602000u);
}

TEST_F(SparseMetricsTest, QueryViaPathCounter) {
...
TEST_F(SparseMetricsTest, LabelStrings) {
...
TEST_F(SparseMetricsTest, RenderContainsAll4Metrics) {
    M().RecordInference(IS::kSuccess);
    M().RecordQueryViaPath(VP::kSparse);
...
TEST_F(SparseMetricsTest, ResetForTestClearsAll) {
    M().RecordInference(IS::kSuccess);
    M().RecordQueryViaPath(VP::kSparse);
