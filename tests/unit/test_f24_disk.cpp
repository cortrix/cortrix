...
TEST(F24ErrorTest, CountMatchesEnumeration) {
    EXPECT_EQ(kF24ErrorCodeCount, 4);
    EXPECT_EQ(std::size(kAll), static_cast<size_t>(kF24ErrorCodeCount));
}

TEST(F24ErrorTest, AllCodesHaveUniqueCxStrings) {
    std::set<std::string> seen;
    for (F24ErrorCode c : kAll) {
        std::string s = F24ErrorCodeString(c);
...
TEST(F24ErrorTest, RegistryMatchesSpecTable) {
    auto chk = [](F24ErrorCode c, const char* code, int http, ErrorCategory cat,
                  bool retry) {
        const F24ErrorInfo& i = GetF24ErrorInfo(c);
    ...
    chk(F24ErrorCode::kDiskFull, "CX_ERR_DISK_FULL", 507,
        ErrorCategory::kPermanent, false);
    chk(F24ErrorCode::kShutdownInProgress, "CX_ERR_SHUTDOWN_IN_PROGRESS", 503,
        ErrorCategory::kTransient, true);
    chk(F24ErrorCode::kHealthCheckFailed, "CX_ERR_HEALTH_CHECK_FAILED", 503,
        ErrorCategory::kTransient, true);
    chk(F24ErrorCode::kBfNotReady, "CX_ERR_BF_NOT_READY", 503,
        ErrorCategory::kTransient, true);
    // Non-retryable code carries no retry_after_ms; retryable ones do.
    EXPECT_FALSE(GetF24ErrorInfo(F24ErrorCode::kDiskFull).retry_after_ms.has_value());
    EXPECT_TRUE(GetF24ErrorInfo(F24ErrorCode::kShutdownInProgress).retry_after_ms.has_value());
}

TEST(F24ErrorTest, DiskFullRequiresFiveStructuredKeys) {
    const auto& keys = RequiredStructuredDataKeys(F24ErrorCode::kDiskFull);
    ...
    EXPECT_TRUE(HasRequiredStructuredData(F24ErrorCode::kDiskFull, complete));
    nlohmann::json missing = {{"disk_usage_ratio", 0.93}};
    EXPECT_FALSE(HasRequiredStructuredData(F24ErrorCode::kDiskFull, missing));
}

TEST(F24ErrorTest, MakeErrorFillsCanonicalFields) {
    auto err = MakeF24Error(F24ErrorCode::kBfNotReady, {{"rebuild_progress", 0.4}});
...
TEST(F24ErrorTest, StatusBridgePreservesCodeToken) {
    Status s = F24Status(F24ErrorCode::kDiskFull, "no space");
    EXPECT_FALSE(s.ok());
...
TEST_F(DiskMonitorTest, MakeDiskFullErrorHasCompleteBody) {
    ...
    ASSERT_TRUE(err.structured_data.has_value());
    EXPECT_TRUE(HasRequiredStructuredData(F24ErrorCode::kDiskFull, *err.structured_data));
