...
// Contextual retrieval (A unified-blocks): ContextualSchemaProvider extends the
...
// registered via the catalog SchemaMigrator.
...
TEST(ContextualSchemaProviderTest, MigrationIdempotent) {
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
    CreateBlocksTable(db);
    ContextualSchemaProvider p;
...
TEST(ContextualSchemaProviderTest, NoBlocksTableStillCreatesLabelTable) {
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
    ContextualSchemaProvider p;
...
TEST(ContextualSchemaProviderTest, V1ToV2UpgradeCreatesLabelTable) {
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
    CreateBlocksTable(db);
    ContextualSchemaProvider p;
...
TEST(ContextualSchemaProviderTest, UnexpectedVersionStepIsError) {
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
    ContextualSchemaProvider p;
...
TEST(ContextualSchemaProviderTest, RegistersAndMigratesViaMigratorUnit) {
    sqlite3* db = nullptr;
    ASSERT_EQ(sqlite3_open(":memory:", &db), SQLITE_OK);
    CreateBlocksTable(db);
    ContextualSchemaProvider p;
