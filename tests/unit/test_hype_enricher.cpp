...
TEST(HypeHyPEEnricherTest, UsableViaBasePointer) {
...
TEST(HypeHyPEEnricherTest, GeneratesKQuestions) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(HypeHyPEEnricherTest, GenerateLlmFailureReturnsError) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(HypeHyPEEnricherTest, GenerateNullClientReturnsError) {
...
TEST(HypeHyPEEnricherTest, GeneratePassesConfiguredTimeout) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(HypeHyPEEnricherTest, GenerateZeroTimeoutKeepsClientDefault) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    llm::LlmCallConfig captured;
...
TEST(HypeHyPEEnricherTest, PromptIncludesParentTextWhenProvided) {
...
TEST(HypeHyPEEnricherTest, ResolveParentTextWithStore) {
...
TEST(HypeHyPEEnricherTest, ResolveParentTextNullStoreIsEmptyNotError) {
    HyPEEnricher e(HyPEConfig{}, std::make_shared<llm::MockLlmClient>(), nullptr);
    auto r = e.ResolveParentText("p1");
...
TEST(HypeHyPEEnricherTest, ResolveParentTextEmptyIdIsEmptyNotError) {
    auto store = std::make_shared<store::MockParentChunkStore>();
...
TEST(HypeHyPEEnricherTest, ResolveParentTextNotFoundSurfacesError) {
    auto store = std::make_shared<store::MockParentChunkStore>();
...
TEST(HypeHyPEEnricherTest, EnrichReturnsStandardResultSuccess) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(HypeHyPEEnricherTest, EnrichDegradesOnLlmFailure) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    llm::ChatCompletionResponse fail;
...
TEST(HypeHyPEParseTest, TrimsAndDropsBlankLines) {
    auto r = HyPEEnricher::ParseQuestions("  q1  \n\n   \n q2 \nq3\n", 3);
    ASSERT_TRUE(r.ok());
    ASSERT_EQ(r.value().size(), 3u);
...
TEST(HypeHyPEParseTest, TooFewIsParseFailed) {
    auto r = HyPEEnricher::ParseQuestions("q1\nq2", 3);  // 2 != 3
    EXPECT_FALSE(r.ok());
...
TEST(HypeHyPEParseTest, TooManyIsParseFailed) {
    auto r = HyPEEnricher::ParseQuestions("q1\nq2\nq3\nq4", 3);  // 4 != 3
    EXPECT_FALSE(r.ok());
    EXPECT_NE(r.status().message().find("CX_ERR_HYPE_QUESTION_PARSE_FAILED"),
              std::string::npos);
...
TEST(HypeHyPEParseTest, EmptyOutputIsInvalidOutput) {
...
TEST(HypeHyPEEnricherTest, GenerateRejectsCountMismatch) {
    auto llm = std::make_shared<llm::MockLlmClient>();
...
TEST(HypeHyPEEnricherTest, GenerateRespectsConfiguredK) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    EXPECT_CALL(*llm, Chat(_, _)).WillOnce(Return(OkChat("q1\nq2\nq3\nq4\nq5")));
    HyPEConfig cfg;
...
TEST(HypeHyPEEnricherTest, PromptOmitsParentSectionWhenEmpty) {
    HyPEEnricher e(HyPEConfig{}, std::make_shared<llm::MockLlmClient>(), nullptr);
...
TEST(HypeHyPEEnricherTest, PromptOmitsDocTitleWhenEmpty) {
    HyPEEnricher e(HyPEConfig{}, std::make_shared<llm::MockLlmClient>(), nullptr);
...
TEST(HypeHyPEEnricherTest, EnrichBatchRunsPerContext) {
    auto llm = std::make_shared<llm::MockLlmClient>();
    EXPECT_CALL(*llm, Chat(_, _))
