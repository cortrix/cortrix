...
class TenantIntegrationTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(TenantIntegrationTest, Register_Flow_CommitsAllThreeTables) {
...
TEST_F(TenantIntegrationTest, Register_Flow_Failure_RollbackAll) {
    sqlite3* db = catalog_.db();
    ASSERT_EQ(sqlite3_exec(db, "BEGIN", nullptr, nullptr, nullptr), SQLITE_OK);
...
TEST_F(TenantIntegrationTest, CrossNsQuery_BatchCheck) {
    sqlite3* db = catalog_.db();
...
TEST_F(TenantIntegrationTest, Admin_HttpQuotaCheck_V1OSS) {
    sqlite3* db = catalog_.db();
...
TEST_F(TenantIntegrationTest, AdminApiKey_TenantCreate_Success) {
...
TEST_F(TenantIntegrationTest, CrossTenant_AclGrant_EnablesRead) {
    sqlite3* db = catalog_.db();
    ASSERT_EQ(sqlite3_exec(db,
