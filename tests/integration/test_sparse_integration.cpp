...
TEST(SparseIntegrationTest, IT1_EmbedSerializeRoundTrip) {
...
TEST(SparseIntegrationTest, IT2_IndexAndQuery) {
    cortrix::OnnxEmbedder emb("", 128);
    ASSERT_TRUE(emb.Init().ok());
...
TEST(SparseIntegrationTest, IT3_DeleteRemovesFromResults) {
...
TEST(SparseIntegrationTest, IT5_FivePathRrfWithRealSparse) {
    cortrix::OnnxEmbedder emb("", 64);
    ASSERT_TRUE(emb.Init().ok());
    SpladeSparseRetriever idx(SpladeConfig{}, ":memory:");
    ASSERT_TRUE(idx.Open().ok());
...
TEST(SparseIntegrationTest, IT7_L2FallbackDegrades) {
...
TEST(SparseIntegrationTest, IT8_CacheHitRateUnderLoad) {
    cortrix::OnnxEmbedder emb("", 64);
    ASSERT_TRUE(emb.Init().ok());
    SpladeSparseRetriever idx(SpladeConfig{}, ":memory:");
    ASSERT_TRUE(idx.Open().ok());
