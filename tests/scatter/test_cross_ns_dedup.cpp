...
// rerank_score source, meta.deduplicated_chunks[] abbreviated multi-source, deduplicated_chunks_count.
...
// Two NS share one content_hash → collapse to the higher-rerank primary.
TEST(CrossNsDedupTest, CollapsesDuplicateAcrossNamespaces) {
    ...
    EXPECT_EQ(items[0].namespace_id, "ns_high");  // primary = highest rerank_score
...
// The abbreviated multi-source array carries each source's (NS, rerank_score).
...
// First-appearance order is preserved (a pre-sorted-by-rerank input stays sorted).
...
// Tie on rerank_score → earliest occurrence is primary (stable).
