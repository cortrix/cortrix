<div align="center">
  ...
  Agent-first semantic storage for durable, queryable context.
  ...
  [Install with an AI Agent](docs/AGENT_QUICKSTART.md) · [Quickstart](docs/QUICKSTART.md) · [Agent access](docs/agent-access.md) · [MCP](cortrix-mcp/README.md) · [OpenAPI](api/openapi.yaml) · [Compatibility](docs/compatibility.md)
</div>

Cortrix is an agent-first semantic storage server for AI agents that need durable, queryable context. It keeps documents, derived blocks, hybrid retrieval, memory, and source context in one local-first service, then exposes that data through MCP, HTTP/OpenAPI, the Python SDK, and framework adapters.

Instead of rebuilding ingestion, retrieval, memory, and error handling inside every application, agents can work through stable, machine-readable contracts while humans keep control of installation, access, and deployment boundaries.

## What Agent-First Means

- **Agents can install and verify Cortrix.** The [AI Agent setup contract](docs/AGENT_QUICKSTART.md) gives a terminal-capable agent an exact release, commit, permission boundary, verification procedure, and final report schema.
- **Agents can choose the access path that fits their runtime.** Cortrix exposes MCP, HTTP/OpenAPI, a Python SDK, framework adapters, and a built-in fixed-flow Agent service.
- **Agents receive actionable failures.** The API describes structured error fields such as `code`, `retryable`, `category`, `retry_after_ms`, and `structured_data` instead of requiring agents to parse free-form messages.
- **Humans keep the boundary.** The primary Quick Start binds to loopback, external LLM roles remain optional, and production or security claims stay behind explicit compatibility checks.

## Start with an AI Agent

Give a terminal-capable AI agent the [Install Cortrix with an AI Agent](docs/AGENT_QUICKSTART.md) contract. It pins the release and commit, limits the run to the loopback-only Docker path, prohibits provider keys and system security changes, and requires the agent to verify readiness, source content, and numeric reranking scores before reporting success.

Prefer to run the commands yourself? Use the [Docker Quick Start](docs/QUICKSTART.md).
...
- `cortrix-skills/`: framework adapters for LangChain, Claude Tools, and OpenAI function calling.
- `cortrix-agent/`: the built-in fixed-flow RAG chat service.
- `web/`: the local Web UI.

## Documentation Map

Start here, with the path that matches how you want to use Cortrix:
...
- [Quickstart](docs/QUICKSTART.md): start Cortrix with Docker, check readiness, and run a source-backed reranked query yourself.
- [Agent access](docs/agent-access.md): choose between HTTP/OpenAPI, MCP, the Python SDK, and the built-in Agent.
- [Cortrix Skills](cortrix-skills/README.md): expose Cortrix operations as tools for LangChain, Claude Tools, and OpenAI function calling.
...
| Framework adapters | `Verification required` | `cortrix-skills` includes LangChain, Claude Tools, and OpenAI function calling adapters; verify the target framework and runtime before deployment. |
...
If you are using a terminal-capable AI Agent, use the [AI Agent setup contract](docs/AGENT_QUICKSTART.md). It pins the release and commit, limits the agent to the same loopback-only Docker path, and requires a structured verification report.
...
Cortrix exposes direct and framework access paths:
...
| Framework adapters | LangChain, Claude Tools, and OpenAI function calling | [cortrix-skills/README.md](cortrix-skills/README.md) |
| Built-in Agent | Local fixed-flow chat over Cortrix storage | [cortrix-agent/README.md](cortrix-agent/README.md) |

See [Agent access](docs/agent-access.md) for the core access paths and their current support status. See [Cortrix Skills](cortrix-skills/README.md) for framework adapter setup and examples.
...
- **Agent surface**: a programmatic path that lets an Agent use Cortrix through HTTP, MCP, the Python SDK, framework adapters, or the built-in Agent service.
